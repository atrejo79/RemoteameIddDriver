...
#include "../RemoteameIddDriver/Public.h"
...
char g_lastMsg[1024];
const char* g_msgHeader = "RemoteameIdd: ";
...
BOOL InstallUpdate(LPCTSTR fullInfPath, PBOOL rebootRequired)
{
    ...
    if (FALSE == UpdateDriverForPlugAndPlayDevices(
        NULL,
        _T("RemoteameIddDriver"),    // match hardware id in the inf file
        fullInfPath,
        INSTALLFLAG_FORCE
...
BOOL DeviceCreate(PHSWDEVICE hSwDevice)
{
    ...
    PCWSTR description = L"Remoteame Idd Driver";

    // These match the Pnp id's in the inf file so OS will load the driver when the device is created    
    PCWSTR instanceId = L"RemoteameIddDriver";
    PCWSTR hardwareIds = L"RemoteameIddDriver\0\0";
    PCWSTR compatibleIds = L"RemoteameIddDriver\0\0";
    ...
    // Create the device
    HRESULT hr = SwDeviceCreate(L"RemoteameIddDriver",
        L"HTREE\\ROOT\\0",
        &createInfo,
        0,
        NULL,
